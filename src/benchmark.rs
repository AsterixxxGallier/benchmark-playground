...
impl BenchmarkStatistics {
    fn new(mut samples: Vec<PreciseDuration>) -> Self {
        ...
        samples.retain(|&duration| (duration.saturating_sub(avg)) <= avg / 10u32);
    ...
    pub(crate) fn report(&self) {
        ...
        if self.avg.total_attos() < 1000 {
            ...
            println!("min: {:>3}.{:0>3} fs", self.min.total_femtos(), self.min.part_attos());
            println!("avg: {:>3}.{:0>3} fs", self.avg.total_femtos(), self.avg.part_attos());
            println!("std: {:>3}.{:0>3} fs", self.std.total_femtos(), self.std.part_attos());
        } else if self.avg.total_picos() < 1000 {
            println!("min: {:>3}.{:0>3} ps", self.min.total_picos(), self.min.part_femtos());
            println!("avg: {:>3}.{:0>3} ps", self.avg.total_picos(), self.avg.part_femtos());
            println!("std: {:>3}.{:0>3} ps", self.std.total_picos(), self.std.part_femtos());
        } else if self.avg.total_nanos() < 1000 {
            println!("min: {:>3}.{:0>3} ns", self.min.total_nanos(), self.min.part_picos());
            println!("avg: {:>3}.{:0>3} ns", self.avg.total_nanos(), self.avg.part_picos());
            println!("std: {:>3}.{:0>3} ns", self.std.total_nanos(), self.std.part_picos());
        } else if self.avg.total_micros() < 1000 {
            println!("min: {:>3}.{:0>3} µs", self.min.total_micros(), self.min.part_nanos());
            println!("avg: {:>3}.{:0>3} µs", self.avg.total_micros(), self.avg.part_nanos());
            println!("std: {:>3}.{:0>3} µs", self.std.total_micros(), self.std.part_nanos());
        } else if self.avg.total_millis() < 1000 {
            println!("min: {:>3}.{:0>3} ms", self.min.total_millis(), self.min.part_micros());
            println!("avg: {:>3}.{:0>3} ms", self.avg.total_millis(), self.avg.part_micros());
            println!("std: {:>3}.{:0>3} ms", self.std.total_millis(), self.std.part_micros());
        } else {
            println!("min: {:>3}.{:0>3} s", self.min.total_seconds(), self.min.part_millis());
            println!("avg: {:>3}.{:0>3} s", self.avg.total_seconds(), self.avg.part_millis());
            println!("std: {:>3}.{:0>3} s", self.std.total_seconds(), self.std.part_millis());
        }
    }
...
pub(crate) struct Bencher {
    overhead: PreciseDuration,
    cycle: PreciseDuration,
}

impl Bencher {
    pub(crate) fn new() -> Self {
        let zero_bencher = Self {
            overhead: PreciseDuration::zero(),
            cycle: PreciseDuration::zero(),
        };

        let overhead = zero_bencher.benchmark(|| random(), |value: u64| value).min;
        let cycle = zero_bencher.benchmark(|| random(), |value: u64| value + 1).min - overhead;
        Self { overhead, cycle }
    }
    ...
    fn benchmark_with_counts<T>(
        ...
        let samples: Vec<_> = (0..benchmarks)
            .map(move |_| {
                (sample(samples, t(), &mut f) / samples as u128).saturating_sub(self.overhead)
            })
            .collect();
